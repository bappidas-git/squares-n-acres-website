...
Status: IN PROGRESS
Last prompt executed: 37 — SEO dashboard, settings, redirects, playbook Next prompt: 38
...
| 37  | SEO dashboard, global SEO settings, redirects manager, playbook | HEAD of this branch (a commit cannot contain its own hash — prompt 38 fills it in) | 2026-09-17 |
...
| `AdminPlaceholderPage` routes                    | **One route left.** `/admin/media` renders `AdminPlaceholderPage` until prompt 39 writes the media library; every other admin route is a real screen. Prompt 30 took the three `pages` routes off the list, 31 the careers and newsletter ones, 33 the articles ones, and **37 the last four SEO routes** (`/admin/seo`, `/admin/seo/settings`, `/admin/seo/redirects`, `/admin/seo/guide`). Nothing may still point at the placeholder after prompt 43.                                                                          | 13–39         |
...
| Blog `Helmet` → `<Seo type="article">`           | `ArticleDetail.jsx` sets `<title>`, the description, the canonical, `robots` (`noindex, nofollow` while previewing or while the article is not published), the Open Graph block and `article:published_time` / `article:modified_time` / `article:section` through `react-helmet-async`. Prompt 38 hands the same record to `<Seo type="article">`, which adds the Twitter card and the `Article` + `BreadcrumbList` + `FAQPage` graph — the last of them from the questions the page already merges (`SafeHtml`'s `onFaqItems`). | 38            |
| Blog index `Helmet` → `<Seo>`                    | `ArticleIndex` — the blog index and the category, tag and author archives — sets the title, the description, the canonical and the `<link rel="alternate" type="application/rss+xml">` that points at the API's `/rss.xml`, and `Pagination` emits `rel=prev/next` from the same object. Prompt 38 replaces them with `<Seo type="articleCategory">` / `<Seo type="author">` and their JSON-LD.                                                                                                                                   | 38            |
