...
| NEW-43                    | **Three colour-contrast failures Lighthouse reports at AA.** (1) `Breadcrumbs` over the locality hero: near-white link text on the hero's composited scrim measures **3.64:1** at 14 px, where AA wants 4.5:1. (2) `LocalitySections_heroStatLabel` on the same scrim: **3.99:1** at 12 px. (3) `TrendingList_rank` on the article index: **1.37:1** at 18 px bold — a pale grey numeral on the surface tint. That one is `aria-hidden` decoration, so the question is whether it should be darker or is genuinely decorative. (1) and (2) are text over a hero photograph, so what axe measured is the scrim over the placeholder the sandbox left behind rather than over a picture; the hazard is real either way and the scrim is the lever. `npm run check:contrast` sees none of the three, because it checks token pairs and these are composites. The measured foreground and background values are in the Lighthouse JSON rather than here: `check:traces` (D17) forbids a hex literal outside the three files that own the palette, and that rule is right even in prose.                                   | 41, Lighthouse mobile on `/localities/whitefield` and `/insights/articles`                                                                                                                                                                                                                                                                                                                      | 42                                                                                                                                                                                                                                                                                                                                                                                                                   |
