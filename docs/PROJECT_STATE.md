...
Status: IN PROGRESS
Last prompt executed: 07 — Mock server: authentication, tokens, RBAC middleware, users, profile Next prompt: 08
...
| 07  | Auth tokens, RBAC middleware, users CRUD and profile          | HEAD of this branch (a commit cannot contain its own hash — prompt 08 fills it in) | 2026-09-15 |
...
`POST /admin/users/bulk` (07). Since prompt 07 every `/api/admin/*` path — hand-written or
generic — answers 401 without a bearer token and 403 outside the §7 matrix.
...
| BUG-03                    | 15 endpoints called by the frontend do not exist on a plain JSON Server (`/auth/*`, `/admin/dashboard`, `/seo/*`, `/properties/slug/:slug`, `/admin/leads/:id/notes`, `/newsletter/subscribe`, `/neighborhoods/active`, `/partners/active`, `/articles/trending`, `/articles/slug/:slug`, `/visits`). **Partially closed in prompts 06–07:** the mock serves generic CRUD for all 28 collections under `/api` and `/api/admin` with the §5 envelope (06) and the five `/auth/*` routes plus `/admin/users` (07); `/admin/dashboard`, `/seo/*`, `/properties/slug/:slug`, `/admin/leads/:id/notes`, `/newsletter/subscribe`, `/articles/trending` and `/articles/slug/:slug` still 404 until prompts 08–09. `/neighborhoods/active`, `/partners/active` and `/visits` are boilerplate paths the contract replaces (11). | master spec, confirmed 01 | 08–09                       |
