...
Status: IN PROGRESS
Last prompt executed: 24 — Property details, part 2 Next prompt: 25
...
| 24  | Property details part 2: the eleven content sections           | HEAD of this branch (a commit cannot contain its own hash — prompt 25 fills it in) | 2026-09-16 |
...
| `src/utils/adapters/legacyProperty.js`         | `toLegacyProperty()` now maps only the handful of names `PropertyListing`'s client-side filter, its sort and `PropertyFilters`' facet lists still read (`type`, `status`, `propertyType`, `propertyTypeId`, `price`, `configuration`, `location.area`/`city`, `developer`). Prompt 23 removed every details-only mapping with the page that read them; prompt 26 replaces the listing with the server-driven engine (D94) and deletes the file.                                                                                                                                        | 26           |
...
| `LeadModalTemp` → `LeadCaptureModal`           | One dialog, parameterised by its `LEAD_SOURCES` value, replaces the four copy-pasted modal state machines the property page carried (ADD-12). It has no requirement fields, no submit throttle (D43) and does not deliver a gated file (BUG-08); prompt 28 unifies every form on the site behind `LeadCaptureModal` and deletes it.                                                                                                                                                                                                                                                    | 28           |
| `SectionPlaceholder` (development only)        | Six of the eighteen section keys still have no component: `documents`, `finance`, `similar` and `enquiry`, which prompt 25 writes, and `video` / `virtualTour`, whose content the gallery at the top of the page already shows as tabs, so what they still need is a navigation target rather than a section. Each renders the `<section id="section-<key>">` wrapper with a dashed "Section content arrives in prompt 25" card **in development builds only**; a production build renders nothing inside the wrapper. Prompt 25 deletes the component and settles the two media keys. | 25           |
...
| BUG-05 (partially closed) | PropertyDetails renders sections with defaults/placeholders (`DEFAULT_BANKS`, `'—'`, `Document`, "Map view available on live version"). **The bank half is closed in 15**: `DEFAULT_BANKS` — six real brands with invented rates — is deleted with `src/config/adminConstants.js`, `FinanceGuide` reads `useBanks()`, and with no active bank the section hides itself instead of inventing lenders. **Prompt 23** rebuilt the page shell without any of them. **Prompt 24 closes the eleven content sections**: overview, highlights, unit configurations, specifications, amenities, floor plans, gallery, construction, builder, location & nearby and FAQs are all data-driven, every legacy section component is deleted, and a fact the record does not carry is a row that is not printed. What is left is the `Document` label of `PropertyDocuments` and the last of `FinanceGuide`, both owned by 25. | master spec, confirmed 01 | 23–25                       |
...
| ADD-12 | `PropertyDetails`: 30 `useState`s (spec said 24), four copy-pasted modal state machines, `handleOpenLeadForm` dead → **the enquiry modal is unreachable**, `EnquiryForm` mounted three times, `dimensionRange` chip always renders, modals without `role="dialog"`/focus trap, `og:site_name` hardcoded to the boilerplate brand **Mostly closed in 23**: the page is rewritten with six `useState`-free section components and one `useApi` call, the four modal state machines are one `LeadModalTemp` over the kit's `Modal` (focus trap, `Escape`, `aria-modal`), every enquiry path is reachable (price card, mobile bar, section nav), `EnquiryForm` is mounted nowhere, the `dimensionRange` chip is gone and the key facts print only what the record carries. The enquiry **section** and the OG tags belong to 25 and 38. | master spec, confirmed 01 | 25, 28, 38 |
...
| NEW-32 | The bottom navigation's new "Saved" item and the "View" link in the shortlist toast both point at `/shortlist`, which has no route yet — a visitor who follows either lands on the 404 page (which carries a search box and links, so it is not a dead end). Prompt 26 writes the page.                                                                                                                                                                                                                                    | 23       | 26                         |
