...
Status: IN PROGRESS
Last prompt executed: 22 — Admin property list Next prompt: 23
...
| 22  | Admin property list: filters, bulk, toggles, CSV export       | HEAD of this branch (a commit cannot contain its own hash — prompt 23 fills it in) | 2026-09-16 |
...
| `src/utils/adapters/legacyProperty.js`         | `toLegacyProperty()` maps the §6.1 record onto the field names `PropertyDetails`, `PropertyListing` and the detail sections still read (`gallery`, `location.area`, `type`, `status`, `priceUnit`, `specialities`, flat SEO fields). Prompt 21 removed the four mappings only the deleted `PropertyForm.jsx` used (`publishStatus`, `propertyTypeName`, `statusLabel`, `category`) and restored the `highlights` → `specialities` half of D40; prompt 22 removed the admin-list ones (`toLegacyProperties()`, `tags`) with the table that read them. Nothing writes through it and no admin screen reads through it any more. Prompts 23–26 rewrite the two public screens against the real shape and delete the file. | 23–26        |
...
| BUG-06                    | `StickyNav` ignores toggles; `visibleSections` logic duplicated; "Construction" targets `construction-specs` **Prompt 21 wrote the one rule both sides will read**: `src/utils/propertySections.js` holds the eighteen `SECTION_DEFINITIONS` (label, description, anchor, `hasData`), `getVisibleSections(property, context)` for the page and its sub-navigation, and `getSectionHints(values, context)` for the admin toggles, with 30 unit tests. Prompt 23 replaces the page’s duplicated `visibleSections`/`sectionFlags` with it, which is when the row closes.                                                                                                                 | master spec, confirmed 01 | 23                          |
