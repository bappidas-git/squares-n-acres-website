...
Status: IN PROGRESS
Last prompt executed: 03 — Purge HOM traces and dead code; strict trace check Next prompt: 04
...
| 03  | Purge HOM traces and dead code; strict trace check           | HEAD of this branch (a commit cannot contain its own hash — prompt 04 fills it in) | 2026-09-15 |
...
| Hex literals in `public/index.html` (`theme-color`) and `public/manifest.json` (`theme_color`, `background_color`) | A meta tag and a web-app manifest cannot reference a CSS variable, so these three literals stay. Prompt 03 did not widen the scanner's hex allow-list for them: they fall under the colour-literal deferral (`colourLiterals: "report"`), which prompt 04 turns into a gate. Prompt 04 must therefore either add the two files to `HEX_ALLOWED_*` or update the literals to the SNA palette and allow them explicitly. | 04           |
| `dev` = `react-scripts start`                                                                                      | Placeholder until the mock server exists; becomes `concurrently` mock + web.                                                                                                                                                                                                                                                                                                                                           | 06           |
| `scripts/check-traces.allow.json`                                                                                  | Exempts `src/theme.js` (whole file) and the eleven `:root` palette and font declarations of `src/assets/styles/global.css` from the trace patterns, and sets `colourLiterals: "report"` so the ~1 500 remaining hex literals are reported but do not fail the check. Prompt 04 replaces the file with `{ "files": [], "lines": {} }`, which removes both exemptions and makes colour literals blocking in one edit.    | 04           |
| `--font-serp` / `--font-mono` in `global.css`                                                                      | Two non-brand font tokens added by prompt 03 so the Google-SERP preview mocks and the code/slug fields keep their faces without an inline `fontFamily`. Prompt 04 owns the final token set and should keep or rename them deliberately.                                                                                                                                                                                | 04           |
...
| BUG-12 (partial)  | ~1 500 hex literals in JS/JSX and CSS modules remain. Cleared in 03: all 31 inline `fontFamily`/`font-family` literals (now `var(--font-*)`), the SweetAlert2 override classes and their `customClass` object. `ErrorBoundary`, `PageLoader` and `BackToTop` still carry inline colours.                                | master spec, measured 01  | 04 (+ every module prompt), 43 |
...
| ADD-24 | `SkeletonLoaders.PropertyCardSkeleton` shows two buttons the card doesn't have (CLS); no `aria-busy`; `PageLoader` prints the boilerplate brand in its display serif                                                                                                                                                                                                                                                                                                 | master spec, confirmed 01 | 03, 04                                          |
