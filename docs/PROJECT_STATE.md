...
| NEW-36 | `PagesListPage`'s "Duplicate" row action cannot succeed: it posts the copy with `slug: ''` and `seo.slug: ''`, and §6.10 types a page's slug as a patterned string that the empty string does not match, so the API answers `422 {slug, seo.slug}` before `resolveSlug` is ever reached. Verified against the running mock with the exact payload the screen builds. The fix is the one prompt 33 applied to `articleService.duplicate`: **omit** both keys rather than send them empty, which is what §5.9 means by "the client sends an empty slug" — the API then derives the slug from the copy's title and de-duplicates it. One line in `src/pages/admin/pages/PagesListPage.jsx`.                                                                                                                                                          | 33 (QA of the article duplicate, which had the same bug) | 45 (QA bug bash: CMS)           |
...
One defect found and recorded rather than fixed here: **NEW-36** — the CMS pages
list's "Duplicate" action posts `slug: ''` and cannot succeed, the same bug the
article duplicate had before this prompt sent the key _absent_ instead of empty.
It is prompt 30's file and one line; booked to 45.
