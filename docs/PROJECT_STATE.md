...
Status: IN PROGRESS
Last prompt executed: 23 — Property details, part 1 Next prompt: 24
...
| 23  | Property details part 1: page shell, gallery, price, shortlist | HEAD of this branch (a commit cannot contain its own hash — prompt 24 fills it in) | 2026-09-16 |
...
| `SectionPlaceholder` (development only)        | The property page renders a `<section id="section-<key>">` wrapper for every visible section so the section navigation has a target and its scroll-spy has something to observe; the content of the eighteen sections is written by prompts 24 and 25. Until then each wrapper holds a dashed "Section content arrives in prompt 24/25" card **in development builds only** — a production build renders nothing inside the wrapper, because a visitor must never be shown a note to the developers. | 24, 25       |
...
| BUG-05 (partially closed) | PropertyDetails renders sections with defaults/placeholders (`DEFAULT_BANKS`, `'—'`, `Document`, "Map view available on live version"). **The bank half is closed in 15**: `DEFAULT_BANKS` — six real brands with invented rates — is deleted with `src/config/adminConstants.js`, `FinanceGuide` reads `useBanks()`, and with no active bank the section hides itself instead of inventing lenders. Prompt 23 rebuilt the page shell without any of them — no `'—'` cell, no "Map view available on live version", and an empty gallery is the monogram rather than a blank box. The section **content** is written by 24/25, which is when the row closes.                          | master spec, confirmed 01 | 23–25                       |
