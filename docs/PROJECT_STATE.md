...
Status: IN PROGRESS
Last prompt executed: 11 — Frontend data layer, hooks, contexts and page rewiring Next prompt: 12
...
| 11  | Frontend data layer, hooks, contexts and page rewiring        | HEAD of this branch (a commit cannot contain its own hash — prompt 12 fills it in) | 2026-09-16 |
...
| ADD-07 (pollers) | Two 30-second pollers on `GET /admin/leads` (`AdminLayout` and `AdminLeads`). The toast half of this row is closed in 04. `LeadNotificationsContext` (D45/D55) makes it one poller. **Halved in 11**: both pollers now ask for `perPage=1` and read `meta.total` instead of downloading the collection. `LeadNotificationsContext` makes it one poller in prompt 12.                                                                                                                                                                                                       | master spec, confirmed 01                                                                                                                           | 12, 29                                                                                                                                                                                                                                                                                                                                                       |
