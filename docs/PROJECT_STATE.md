...
Status: IN PROGRESS
Last prompt executed: 25 — Property details, part 3 Next prompt: 26
...
| 25  | Property details part 3: documents, finance, similar, enquiry  | HEAD of this branch (a commit cannot contain its own hash — prompt 26 fills it in) | 2026-09-17 |
...
| `LeadModalTemp` → `LeadCaptureModal`           | One dialog, parameterised by its `LEAD_SOURCES` value, replaces the four copy-pasted modal state machines the property page carried (ADD-12). Prompt 25 gave it `successTitle`/`successAction`, which is how the documents section hands the file over after the lead (BUG-08); what it still lacks is the requirement fields, the submit throttle (D43) and `<label>`s on its boxes (ADD-09). Prompt 28 unifies every form on the site behind `LeadCaptureModal` and deletes it. | 28           |
