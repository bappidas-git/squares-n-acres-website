...
| POST | `/leads` | public | Capture a lead from any form on the site; honeypot and rate limited | — | `lead.create` | `LeadCreated` | Creates the lead with `status:new`, the default priority, a `created` activity and a `propertySnapshot`; increments the property `enquiryCount`; maps legacy sources; a repeat enquiry (open lead, same number, ≤ 30 days) goes to that lead’s owner, else listing-advisor or round-robin assignment when enabled; honeypot; rate limited; answers `access`, the token that opens the named listing’s gated files |
...
| Method | Path                              | Auth/role       | Purpose                                                              | Query                                                                                                                                                                                                                                                                                                                                                                                                  | Body schema       | Response shape  | Side effects                                                                                                                                      |
| ------ | --------------------------------- | --------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET | `/admin/dashboard` | any role | Role-aware dashboard aggregates, trends and recent activity | `range` (7, 30 or 90 days of trend; 30 by default) | — | `DashboardData` | Overdue follow-ups first, each with `isOverdue`, and `overdueCount` (prompt 51) |
| GET | `/admin/properties` | any role | List properties for the admin table | `page`, `perPage`, `sort`, `order`, `q`, `isActive`, `ids`, `listingType`, `segment`, `propertyTypeId`, `localityId`, `cityId`, `constructionStatus`, `availability`, `bedrooms`, `minPrice`, `maxPrice`, `minArea`, `maxArea`, `areaUnit`, `furnishing`, `facing`, `developerId`, `amenityIds`, `badgeIds`, `isFeatured`, `isVerified`, `reraRegistered`, `possessionBy`, `seoScoreBand`, `createdBy`, `agentId` | — | `PropertyList` | — |
...
| PATCH | `/admin/leads/:id` | any role | Change status, priority, assignee, follow-up, lost reason or the contact details | — | `lead.patch` | `Lead` | A change of `name`, `phone`, `email` or `requirement` appends one `details-updated` activity; sales change those on their own leads only (prompt 51) |
...
| POST | `/admin/leads/:id/activities` | any role | Log a call, a WhatsApp, a site visit, a meeting or another conversation | — | `lead.activity` | `Lead` | Appends a typed activity (`call-logged` …) with the outcome in its description and the note on the entry (prompt 51) |
| DELETE | `/admin/leads/:id/notes/:noteId`  | any role        | Delete one note from a lead                                          | —                                                                                                                                                                                                                                                                                                                                                                                                      | —                 | `Lead`          | Removes the note; the activity stays                                                                                                              |
| GET | `/admin/leads/export` | any role | CSV export of the filtered lead list, scoped like the list endpoint | `q`, `status`, `source`, `priority`, `assignedTo`, `propertyId`, `from`, `to`, `followUp`, `idleDays` | — | `Csv` | UTF-8 BOM CSV, `Content-Disposition: attachment; filename="leads-<yyyy-mm-dd>.csv"` |
...
A locality's `name` is unique within its city, case and spacing aside: a create, or a write that changes the name or the city, into a name the city already has is **422** on `name` (QA-60).

`sort=category` on `GET /admin/amenities` orders the categories as the site groups them — `AMENITY_CATEGORIES`: basic, lifestyle, safety, sports, kids, eco, convenience, commercial — then by `order`, not alphabetically (QA-60).
...
| GET | `/admin/team` | admin · manager | List team members for the admin table; each row carries `listingCount` (prompt 51) | `page`, `perPage`, `sort`, `order`, `q`, `isActive`, `ids`, `showOnAbout` | — | `TeamMemberList` | — |
...
| Method | Path                    | Auth/role       | Purpose                                                                                        | Query                                                                                                 | Body schema          | Response shape       | Side effects                                                                                                                                                                                                                             |
| ------ | ----------------------- | --------------- | ---------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | -------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/admin/media`          | admin · manager | List media items for the library grid; `meta.folders` is `[{ name, count }]` for the folders the other filters leave something in, `meta.unfiled` counts the files in none, `usage=unused` keeps the files nothing shows, and `q` also reads the address and the tags | `page`, `perPage`, `sort`, `order`, `q`, `isActive`, `ids`, `type`, `provider`, `folder`, `unfiled`, `usage`, `withUsage` | —                    | `MediaList`          | `withUsage=true` adds `usedIn[]` to every row of the page — one request for the page rather than one per card; `meta.folders` is the folders holding a file every other filter lets through, whatever the page, each with its count (QA-63, prompt 51) |
| POST   | `/admin/media`          | admin · manager | Create a media item                                                                            | —                                                                                                     | `media.create`       | `Media`              | Metadata only — no binary crosses this API (D12). `provider`, `type` and `format` are inferred from the URL when the client omits them. `url` is unique and at most 500 characters (422 on `url`, QA-63) |
...
| PATCH  | `/admin/media/:id`      | admin · manager | Update the given fields of a media item (toggles, order, SEO panel)                            | —                                                                                                     | `media.patch`        | `Media`              | Writes only what it sends — nothing is inferred from the address (QA-63) |
| DELETE | `/admin/media/:id`      | admin · manager | Delete a media record; 409 listing `usedIn` when the file is still in use, unless `force=true` | `force`                                                                                               | —                    | `Null`               | 409 with `data.usedIn` (and `data.usedBy`) when anything a visitor can see still shows the file — see "Media" below; `?force=true` removes the record anyway. The asset itself is never deleted — the API never held it (D12, prompt 39 §5) |
| POST   | `/admin/media/bulk`     | admin · manager | Apply one action to several media items; a delete is all or nothing — a 409 naming every file still in use (`data.refused`), unless `force=true`; `move` refiles them under `payload.folder` (`null` for none) and answers `missing` for ids that matched nothing | `force` | `bulk`               | `BulkResult`         | delete — refused whole when any selected file is still in use (QA-63); move — `{ affected, missing }`, the files that exist are moved (prompt 51) |
| POST   | `/admin/media/folders/rename` | admin · manager | Refile every media record of a folder (and of the folders inside it) under another name; 422 on `to` with `data.existing` when that name holds files, unless `merge: true`. Records only — Cloudinary paths do not change | — | `media.renameFolder` | `MediaFolderRename` | Rewrites `folder` (and `updatedAt`) on every record of the folder; `url` and `publicId` stay, so no Cloudinary asset moves (prompt 51) |
...
| PUT    | `/admin/users/:id`      | admin           | Replace a user with the full record from the form                                              | —                                                                                                     | `user.update`        | `User`               | Keeps your own role; a `password` (8+, a letter and a digit) signs the account out everywhere                                                                                                                                                                                |
| PATCH  | `/admin/users/:id`      | admin           | Update the given fields of a user (the Active switch, a password reset)                                  | —                                                                                                     | `user.patch`         | `User`               | 422 on a change to your own role or to the last active admin; a `password` signs the account out everywhere                                                                                                                              |
| DELETE | `/admin/users/:id`      | admin           | Delete a user; their leads are unassigned                                                     | —                                                                                                     | —                    | `Null`               | Unassigns their leads and revokes their tokens; 422 on yourself or the last active admin                                                                                                                                                                               |
| POST   | `/admin/users/bulk`     | admin           | Apply one action to several users                                                              | —                                                                                                     | `bulk`               | `BulkResult`         | activate · deactivate · delete; `affected` counts accounts that changed; one 422 refuses the batch                                                                                                                                                                         |

**Media (QA-63).** Rules the table has no room for; the formulas are in the
business rules, "Media library":

- **Where a file is used** is looked for in every record a visitor can see it on
  — listings, articles, pages, localities, developers, banks, authors, team
  members, partners, testimonials, FAQs, job openings, the site settings and the
  SEO settings — as a whole address: one followed by more of a path is another
  file. The list works it out for the rows of its page only.
- **`meta.folders`** on `GET /admin/media` is the folders that hold a file
  every _other_ filter lets through, sorted, whatever the page — the Folder
  filter's options: every folder when nothing else is chosen, the folders that
  hold documents under `type=document`. Since prompt 51 each is
  `{ name, count }` — the rail prints "properties (284)" — and `meta.unfiled`
  counts the files in no folder. A lock-step change: the admin reads both
  shapes, so an API still answering plain names shows the folders without
  their counts.
- **`unfiled=true`** keeps the files in no folder; **`usage=unused`** keeps the
  files nothing a visitor can see shows — the search a delete asks first, so
  every file it lists deletes without a 409.
- **Moving and renaming folders** changes records only. The bulk `move` sets
  `folder` on the files that exist and lists the ids it did not find in
  `data.missing`; `POST /admin/media/folders/rename` refiles a folder and the
  folders inside it (`projects` → `archive` takes `projects/aurelia` to
  `archive/aurelia`), refusing a name in use (422 on `to`, `data.existing:
  { name, count }`) unless `merge: true`. A record keeps its `url` and
  `publicId`: the Cloudinary asset stays at the path it was uploaded to.
- **A folder is filed clean**: its segments trimmed, no slash at either end and
  none doubled (`" /projects//aurelia/ "` is `projects/aurelia`), blank as `null`.
- **`q`** reads the alt text, the title, the folder, the public id, the address
  and the tags.
- **`url` is unique and at most 500 characters**: a second record for the same
  file is 422 on `url`. Tags are trimmed, blanks dropped, each kept once.
- **A `PATCH` infers nothing** from the address; a `POST` or `PUT` that omits
  `provider`, `type` or `format` still has them worked out.
- **A bulk delete is all or nothing**: one file still in use refuses the batch
  with 409, `data.refused[]` naming each, unless `?force=true`.
