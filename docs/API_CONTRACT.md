...
| Method | Path                    | Auth/role       | Purpose                                                                                        | Query                                                                                                 | Body schema          | Response shape       | Side effects                                                                                                                                                                                                                             |
| ------ | ----------------------- | --------------- | ---------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | -------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/admin/media`          | admin · manager | List media items for the library grid; `meta.folders` names every folder in the library, and `q` also reads the address and the tags | `page`, `perPage`, `sort`, `order`, `q`, `isActive`, `ids`, `type`, `provider`, `folder`, `withUsage` | —                    | `MediaList`          | `withUsage=true` adds `usedIn[]` to every row of the page — one request for the page rather than one per card; `meta.folders` is every folder in the library, whatever the filters (QA-63) |
...
- **`meta.folders`** on `GET /admin/media` is every folder in the library,
