...
| Method | Path                    | Auth/role       | Purpose                                                                                        | Query                                                                                                 | Body schema          | Response shape       | Side effects                                                                                                                                                                                                                             |
| ------ | ----------------------- | --------------- | ---------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | -------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/admin/media`          | admin · manager | List media items for the library grid; `meta.folders` names the folders the other filters leave something in, and `q` also reads the address and the tags | `page`, `perPage`, `sort`, `order`, `q`, `isActive`, `ids`, `type`, `provider`, `folder`, `withUsage` | —                    | `MediaList`          | `withUsage=true` adds `usedIn[]` to every row of the page — one request for the page rather than one per card; `meta.folders` is the folders holding a file every other filter lets through, whatever the page (QA-63) |
...
| POST   | `/admin/media/bulk`     | admin · manager | Apply one action to several media items; a delete is all or nothing — a 409 naming every file still in use (`data.refused`), unless `force=true` | `force` | `bulk`               | `BulkResult`         | delete — the only action a media item has; refused whole when any selected file is still in use (QA-63) |
...
- **`meta.folders`** on `GET /admin/media` is the folders that hold a file
  ...
  hold documents under `type=document`.
