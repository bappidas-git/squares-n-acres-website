 ...
 * of the listing is answered with (QA-62).
 *
 * The form sends the version it read; the API refuses it when the listing has
 ...
 *     their version (`rebase.js`), not the whole form as it was;
 *   - "Save mine anyway" writes this version over theirs, knowingly;
 *   - "Keep editing" closes the question and sends nothing.
 *
 * @param {object} props
 * @param {{updatedAt?: string, updatedBy?: {name?: string}|null, message?: string}|null} props.conflict
...
export default function ConflictDialog({
  ...
  const who = conflict?.updatedBy?.name || 'Somebody else';
  ...
  return (
    <Modal
      ...
      dismissible={!busy}
      title="Somebody else saved this listing"
      size="sm"
      footer={
        <>
