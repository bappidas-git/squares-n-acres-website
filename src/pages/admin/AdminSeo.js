  ...
  <Tooltip title={hasValue ? `${label} set` : `${label} missing`} arrow>
    <Box
      sx={{
        ...
        height: 8,
        borderRadius: '50%',
        bgcolor: hasValue ? 'var(--color-success)' : 'var(--color-error)',
...
const GooglePreview = ({ title, description, slug }) => (
  ...
  >
    ...
    >
      {PREVIEW_HOST}/properties/{slug || 'property-slug'}
    </Typography>
    <Typography
      sx={{
        fontSize: '0.8125rem',
    ...
    >
      {description ||
        'Meta description will appear here. Add a compelling description to improve click-through rates.'}
    </Typography>
  </Paper>
);
            ...
            {check.issues.length > 0 && (
              <Box sx={{ mt: 0.5 }}>
                {check.issues.map((issue, issueIdx) => (
                  <Typography
                    key={issueIdx}
                    sx={{ fontSize: '0.6875rem', color: 'var(--color-text-muted)', pl: 1 }}
                  >
                    — {issue}
                  </Typography>
                ))}
...
const AdminSeo = () => {
  ...
  const stats = useMemo(() => {
    ...
    return { total, complete, partial, avgScore };
  }, [properties]);
    ...
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      ...
      >
        {statCards.map((stat) => (
          <Paper
            key={stat.label}
            elevation={0}
            sx={{ p: 2.5, borderRadius: 2, border: '1px solid var(--color-surface)' }}
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              ...
              <Box>
                <Typography
                  sx={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--color-charcoal)' }}
                >
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center">
                        <StatusDot
                          hasValue={!!property.seoDescription?.trim()}
                          label="Description"
                        />
                      </TableCell>
                      <TableCell align="center">
      ...
      >
        ...
        <DialogContent sx={{ pt: 3 }}>
          ...
          <Box sx={{ mb: 3 }}>
            <Typography
              sx={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--color-text)', mb: 1 }}
            >
              SEO Title
              <Typography
                component="span"
                sx={{
                  fontSize: '0.6875rem',
                  color: getTitleLenColor(editForm.seoTitle.length),
                  ml: 1,
                }}
              >
                {editForm.seoTitle.length}/{SEO_LIMITS.TITLE_OPTIMAL_MAX} characters
                {editForm.seoTitle.length > 0 &&
                  editForm.seoTitle.length < SEO_LIMITS.TITLE_OPTIMAL_MIN &&
                  ' (too short)'}
                {editForm.seoTitle.length > SEO_LIMITS.TITLE_MAX_LENGTH && ' (too long)'}
                {editForm.seoTitle.length >= SEO_LIMITS.TITLE_OPTIMAL_MIN &&
                  editForm.seoTitle.length <= SEO_LIMITS.TITLE_OPTIMAL_MAX &&
                  ' (optimal)'}
              </Typography>
            </Typography>
            <TextField
          ...
          <Box sx={{ mb: 3 }}>
            ...
            >
              ...
              <Typography
                ...
                sx={{
                  fontSize: '0.6875rem',
                  color: getDescLenColor(editForm.seoDescription.length),
                  ml: 1,
                }}
              >
                {editForm.seoDescription.length}/{SEO_LIMITS.DESC_OPTIMAL_MAX} characters
                {editForm.seoDescription.length > 0 &&
                  editForm.seoDescription.length < SEO_LIMITS.DESC_OPTIMAL_MIN &&
                  ' (too short)'}
                {editForm.seoDescription.length > SEO_LIMITS.DESC_MAX_LENGTH && ' (too long)'}
                {editForm.seoDescription.length >= SEO_LIMITS.DESC_OPTIMAL_MIN &&
                  editForm.seoDescription.length <= SEO_LIMITS.DESC_OPTIMAL_MAX &&
                  ' (optimal)'}
              </Typography>
            </Typography>
            <TextField
          ...
          <Paper
            variant="outlined"
            sx={{ p: 2, borderRadius: 2, mb: 3, border: '1px solid var(--color-border)' }}
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
              <Icon
                icon="mdi:share-variant"
                style={{ fontSize: 18, color: 'var(--color-primary-dark)' }}
              />
              <Typography
                sx={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--color-text)' }}
              >
                Open Graph Tags
              </Typography>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <TextField
                ...
                onChange={(e) =>
                  setEditForm((prev) => ({ ...prev, ogDescription: e.target.value }))
                }
          ...
          <Box>
            ...
            >
              ...
              {editForm.schemaMarkup &&
                (() => {
                  try {
                    JSON.parse(editForm.schemaMarkup);
                    return (
                      <Chip
                        label="Valid JSON"
                        size="small"
