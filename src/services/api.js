...
// Base URL — required, no fallback: the backend is chosen only by this variable
...
const transformPropertyPayload = (data) => {
  ...
  if (data.location) {
    ...
    if (data.location.address !== undefined)
      payload.location_address = data.location.address || null;
  }
  ...
  if (data.constructionTimeline !== undefined)
    payload.constructionTimeline = data.constructionTimeline;
  ...
  return {
    ...
    area:
      fp.area != null
        ? String(fp.area)
        : fp.carpet_area != null
          ? String(fp.carpet_area)
          : fp.super_area != null
            ? String(fp.super_area)
            : '',
    price:
      fp.price != null
        ? String(fp.price)
        : fp.base_price != null
          ? String(fp.base_price)
          : fp.total_price != null
            ? String(fp.total_price)
            : '',
...
const normalizePropertyResponse = (data) => {
  ...
  const rawFloorPlans =
    data.floorPlans || data.floor_plans || data.floorPlan || data.floor_plan || [];
  ...
  const seoObj =
    data.seo && typeof data.seo === 'object' && !Array.isArray(data.seo) ? data.seo : {};
  ...
  return {
    ...
    seoTitle:
      data.seoTitle || data.meta_title || data.seo_title || seoObj.meta_title || seoObj.title || '',
    seoDescription:
      data.seoDescription ||
      data.meta_description ||
      data.seo_description ||
      seoObj.meta_description ||
      seoObj.description ||
      '',
    seoKeywords: (() => {
      const kw =
    ...
    twitterCard:
      data.twitterCard || data.twitter_card || seoObj.twitter_card || 'summary_large_image',
    location,
    dimensionRange,
    gallery,
    amenities,
...
export const propertyService = {
  ...
  getFeatured: async () => {
    ...
    if (hasNonFeatured && normalized.length > 0) {
      return normalized.filter((p) => Array.isArray(p.tags) && p.tags.includes('featured'));
    }
    return normalized;
  },
  ...
  getByStatus: async (status, params = {}) => {
    ...
    return items
      ...
      .filter((p) => !!p.isActive && p.status === status && p.publishStatus !== 'draft');
  },
...
export const articleService = {
  ...
  getByIds: async (ids) => {
    ...
    const results = await Promise.allSettled(ids.map((id) => apiClient.get(`/articles/${id}`)));
